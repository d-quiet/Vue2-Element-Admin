        ...
        <el-form ref="addEmployee" :model="formData" :rules="rules" label-width="120px">
            ...
            <el-form-item label="入职时间" prop="timeOfEntry">
                <el-date-picker v-model="formData.timeOfEntry" style="width:50%" placeholder="请选择入职时间"
                    :picker-options="pickerOptions" @change="timeChoose" />
            </el-form-item>
            ...
            <el-form-item label="转正时间" prop="correctionTime">
                <el-date-picker v-model="formData.correctionTime" style="width:50%" placeholder="请选择转正时间"
                    :picker-options="pickerOptionsRegular" />
            </el-form-item>
        </el-form>
        <!-- footer插槽 -->
...
export default {
    data() {
        return {
            ...
            pickerOptions: {
                disabledDate(date) {
                    // 今天之前被禁用
                    // console.log(new Date(new Date().setHours(0, 0, 0, 0))) // 获取当天零点时间
                    return date.getTime() < new Date(new Date().setHours(0, 0, 0, 0)).getTime()
                }
            },
            pickerOptionsRegular: {
                // 入职之前的时间被禁用
                disabledDate: (date) => {
                    // console.log(this)
                    return this.disabledBool(date)
                }
            },
            rules: {
    ...
    methods: {
        ...
        timeChoose() {
            this.formData.correctionTime = this.formData.timeOfEntry
        },
        // 设置禁用入职之间的事件
        disabledBool(date) {
            const time = date
            const timeOfRegular = this.formData.timeOfEntry
            return time < timeOfRegular
        },
        btnCancel() {
            // 重置原来的数据
            this.formData = {
